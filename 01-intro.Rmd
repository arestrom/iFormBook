...
One case where the API can avoid a great deal of pain is in constructing large filtered option lists. The prime example for spawning ground surveys are the drop-downs for survey start and stop points along a given stream. Lists with thousands of elements can be constructed and uploaded to the iFormBuilder cloud in seconds. The [iformr](https://github.com/arestrom/iformr) package provides a set of functions that allows for using R scripts to automate many of these types of tasks. Contact me if you need help with this.
