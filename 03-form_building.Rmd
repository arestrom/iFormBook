...
The `clip_status` data element provides a good example of using JavaScript code to take `species` into account in determining if the field should be displayed or not. The *Condition Value* code in this case is a little more complicated than in previous elements:  
...
If a coded-wire tag is detected we would want to send the head for to the WDFW CWT tag-lab for processing. We would also want to record the head label number. The `cwt_label` data element allows you to manually enter the label number. If you are using labels that have a pre-recorded barcode label you could change this element to a *Manatee Works* barcode element instead, and simply scan the label rather than enter the data manually. The *Condition Value* for this element specifies that it would only become visible if: `Number(fish_count) == 1 && ZCDisplayKey_cwt_detected == "beep"`.
