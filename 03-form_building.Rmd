...
In Chapter 2 we looked at how the example *spawning_ground* form appeared on your mobile device. In Chapter 3 we will shift focus to the iFormBuilder web interface and learn how elements can be defined, validated, conditionally hidden, and assigned default values. 
...
Make sure you paste the code into the JavaScript panel for your *parent* form, not the *subform*. 

You then go back to the form builder interface and create a read-only element in your parent form by clicking on the *Read Only* widget in the left-hand pane. 
...
Aside from not having to remember the `sort_order` values in order to understand what is actually being stated, using *Key Values* or *Labels* also avoids having to change the statements if you decide to edit the option list and inadvertantly shuffle the `sort_order` values assigned to each option. 
...
## Survey method (core field)

The **survey_method** field is another core element. If you click on the green *Option Lists* bar you will see there are only three options listed: *Foot, Raft, and Helicopter*. Several additional options that you may want to add are listed in the **SpawningGroundDataDictionary** mentioned in chapter 2. If you need to add an option, for example *Snorkle*, you would simply click on the plus sign in the *Actions* column of the option list. A new row will appear, and a new sequential *sort_order* number will be automatically assigned. You would then enter *Snorkle* in the *Label* column, and a lower case *snorkle* in the *Key Value* column. The *Key Value* name should always be lower case. Underscores can be used for compound names, such as *new_redd*. Don't forget to click the save button before closing the *Option Lists* window. 
